...
#### EC-3.6: 버전 전략 관련 엣지 케이스

##### EC-3.6.1: 잘못된 버전 전략
- **입력**: `version_strategy = "invalid"`
- **처리**: GitHub Issues 자동 생성
- **담당자**: `dungsil`에게 할당
- **해결**: 올바른 값으로 수정 후 재실행

##### EC-3.6.2: semantic 전략과 비-GitHub 저장소
- **상황**: GitLab 또는 비-GitHub 저장소 사용
- **처리**: 에러 발생 및 기본 전략 폴백
- **대응**: `natural` 또는 `default` 전략 사용

##### EC-3.6.3: ls-remote 응답 없음 (natural 전략)
- **원인**: Git 서버 일시적 응답 없음
- **처리**: 재시도 로직 적용
- **실패 시**: 기본 브랜치 사용

##### EC-3.6.4: API 레이트 리밋 (semantic 전략)
- **증상**: GitHub API 403 에러
- **처리**: 대기 후 재시도
- **제한**: 시간당 5,000 요청 제한

##### EC-3.6.5: 잘못된 버전 형식
- **semantic**: `v1.2.3` 외 형식 처리
- **natural**: 숫자가 아닌 버전 처리
- **결과**: 정렬 순서 예상치 못할 수 있음
