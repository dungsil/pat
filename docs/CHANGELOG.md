...
### 단어사전 업데이트 시 번역 무효화 로직 재구현 (PR #181)
**날짜:** 2025-12-05
...
### 번역 거부 항목 자동 GitHub Issues 등록 (PR #173)
**날짜:** 2025-12-03
...
### 번역 거부 시 중간 저장 및 Graceful Exit (PR #168)
**날짜:** 2025-12-01
...
### 검증 시스템 False Positive 수정 (PR #174, #171)
...
**해결:**
- 리터럴 텍스트 감지 로직 개선 (PR #174)
- 프롬프트에 변수 구분자 형식 변환 방지 규칙 추가 (PR #171)
...
- 지난 7일간 변경사항 자동 수집
- `[skip ci]` 커밋 자동 제외
- 커밋 메시지의 특수문자로 인한 구문 오류 수정 (PR #164)
...
### "요청하신 대로" 검증 False Positive 수정 (PR #165)
**날짜:** 2025-12-01
...
**해결:**
- `§X$variable$§!` 패턴 지원 (PR #160)
- 섹션 기호 색상 코드 + 파운드 변수 조합 지원: `§H£energy£`, `§BA£variable£` (PR #161)
- 변수만으로 구성된 텍스트 감지 개선으로 AI 수정 방지 (PR #163)
...
### Upstream 업데이트 도구 필터링 기능 (PR #155)
**날짜:** 2025-11-24
...
**해결:**
- `PRAGMA synchronous=NORMAL` 설정 (PR #145)
- 지수 백오프를 사용한 재시도 로직 추가 (PR #138)
- I/O 오류 시 디스크 사용률 표시 (PR #141)
...
**해결:**
- GitHub Releases API 사용 및 git ls-remote 폴백으로 최신 태그 감지 (PR #133)
- GitHub Releases API를 통한 시맨틱 버전 감지 (PR #132)
- Shallow clone에서 버전 태그 존중 (PR #130)
...
변경 사항에 대한 자세한 내용은 각 문서를 참조하거나 GitHub PR을 확인하세요:
- [GitHub 리포지토리](https://github.com/dungsil/paradox-auto-translate)
- [Pull Requests](https://github.com/dungsil/paradox-auto-translate/pulls?q=is%3Apr)
- [이슈](https://github.com/dungsil/paradox-auto-translate/issues)
